 ...
 */
final readonly class ShopListingWithAssociations
{
    /**
     ...
     * @param ShopShippingProfile|null $shipping_profile An array of data representing the shipping profile resource.
     * @param User|null $user Represents a single user of the site
     * @param Shop|null $shop A shop created by an Etsy user.
     * @param list<ListingImage>|null $images Represents a list of listing image resources, each of which contains the reference URLs and metadata for an image
     * @param list<ListingVideo>|null $videos The single video associated with a listing.
     * @param ListingInventory|null $inventory An enumerated string that attaches a valid association. Default value is null.
     * @param list<ShopProductionPartner>|null $production_partners Represents a list of production partners for a shop.
     * @param list<string>|null $skus A list of SKU strings for the listing. SKUs will only appear if the requesting user owns the shop and a valid matching OAuth 2 token is provided. When requested without the token it will be an empty array.
     * @param ListingTranslations|null $translations A map of translations for the listing. Default value is a map of all supported languages keyed to null.
     * @param int|null $views The number of times the listing has been viewed. This value is tabulated once per day and **only for active listings**, so the value is not real-time. If `0`, the listing has either not been viewed, not yet tabulated, wa...
     * @param ListingBuyerPrice|null $buyer_price The buyer-facing price for a listing, including VAT, inclusive shipping (UK), and active promotions. Requires buyer_country parameter. Shows base_price, shipping_cost, original_price (display price), and discounted_price...
     */
    public function __construct(
        ...
        public ?ShopShippingProfile $shipping_profile = null,
        public ?User $user = null,
        public ?Shop $shop = null,
        public ?array $images = null,
        public ?array $videos = null,
        public ?ListingInventory $inventory = null,
        public ?array $production_partners = null,
        public ?array $skus = null,
        public ?ListingTranslations $translations = null,
        public ?int $views = null,
        public ?EtsyModulesListingPersonalizationApiResourcesOpenApiListingPersonalization $personalization = null,
        public ?ListingBuyerPrice $buyer_price = null,
    ) {
    }
    ...
    public static function fromArray(array $data): self
    {
        return new self(
            ...
            shipping_profile: isset($data['shipping_profile']) && is_array($data['shipping_profile']) ? ShopShippingProfile::fromArray($data['shipping_profile']) : null,
            user: isset($data['user']) && is_array($data['user']) ? User::fromArray($data['user']) : null,
            shop: isset($data['shop']) && is_array($data['shop']) ? Shop::fromArray($data['shop']) : null,
            images: isset($data['images']) && is_array($data['images'])
                ? array_values(array_map(static fn (array $i): ListingImage => ListingImage::fromArray($i), $data['images']))
                : null,
            videos: isset($data['videos']) && is_array($data['videos'])
                ? array_values(array_map(static fn (array $i): ListingVideo => ListingVideo::fromArray($i), $data['videos']))
                : null,
            inventory: isset($data['inventory']) && is_array($data['inventory']) ? ListingInventory::fromArray($data['inventory']) : null,
            production_partners: isset($data['production_partners']) && is_array($data['production_partners'])
                ? array_values(array_map(static fn (array $i): ShopProductionPartner => ShopProductionPartner::fromArray($i), $data['production_partners']))
                : null,
            skus: isset($data['skus']) ? (array) $data['skus'] : null,
            translations: isset($data['translations']) && is_array($data['translations']) ? ListingTranslations::fromArray($data['translations']) : null,
            views: isset($data['views']) ? (int) $data['views'] : null,
            personalization: isset($data['personalization']) && is_array($data['personalization']) ? EtsyModulesListingPersonalizationApiResourcesOpenApiListingPersonalization::fromArray($data['personalization']) : null,
            buyer_price: isset($data['buyer_price']) && is_array($data['buyer_price']) ? ListingBuyerPrice::fromArray($data['buyer_price']) : null,
        );
    }
    ...
    public function toArray(): array
    {
        ...
        if ($this->shipping_profile !== null) {
            $data['shipping_profile'] = $this->shipping_profile->toArray();
        }
        if ($this->user !== null) {
        ...
        if ($this->images !== null) {
            $data['images'] = array_map(static fn (ListingImage $i): array => $i->toArray(), $this->images);
        }
        if ($this->videos !== null) {
            $data['videos'] = array_map(static fn (ListingVideo $i): array => $i->toArray(), $this->videos);
        }
        if ($this->inventory !== null) {
            $data['inventory'] = $this->inventory->toArray();
        }
        if ($this->production_partners !== null) {
            $data['production_partners'] = array_map(static fn (ShopProductionPartner $i): array => $i->toArray(), $this->production_partners);
        }
        if ($this->skus !== null) {
            $data['skus'] = $this->skus;
        }
        ...
        if ($this->personalization !== null) {
            $data['personalization'] = $this->personalization->toArray();
        }
        if ($this->buyer_price !== null) {
            $data['buyer_price'] = $this->buyer_price->toArray();
        }
